...
class TopRatedLocalRepository(private val database: MovieDatabase) : MovieRepository<MovieEntity> {
    override fun getMovies(): Single<List<MovieEntity>> {
        return Single.just(database.movieDao().getAll(CATEGORY_TOP_RATED_MOVIES))
    }
}